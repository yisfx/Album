...
function urlBuilder(page, restfulRoute = null, para) {
    let config = route_config_1.RouteConfig[page];
    let url = null;
    if (!!restfulRoute)
        url = site_config_1.default.domain + config.route.replace(":route", restfulRoute);
    else
        url = site_config_1.default.domain + config.route;
    if (!!para) {
        url += "?";
        for (let key in para) {
            url += key + "&" + para[key];
